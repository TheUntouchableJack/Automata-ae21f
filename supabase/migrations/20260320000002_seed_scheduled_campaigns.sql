-- Seed scheduled/paused campaigns and AI recommendations for testing
-- Also adds 'paused' to the status CHECK constraint if not already there

-- 1. Update CHECK constraint to include 'paused'
DO $$
BEGIN
    -- Drop old constraint and add new one with 'paused'
    ALTER TABLE app_message_batches DROP CONSTRAINT IF EXISTS app_message_batches_status_check;
    ALTER TABLE app_message_batches ADD CONSTRAINT app_message_batches_status_check
        CHECK (status IN ('draft', 'scheduled', 'sending', 'sent', 'partially_sent', 'failed', 'cancelled', 'paused'));
...
BEGIN
    ...
    -- Seed AI recommendations (if table exists)
    BEGIN
    ...
    RAISE NOTICE 'Seeded campaigns and recommendations for org %', v_org_id;
END $$;
