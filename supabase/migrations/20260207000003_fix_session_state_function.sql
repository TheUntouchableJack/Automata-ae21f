-- Migration: Fix get_session_discovery_state type casting
-- Fixes UUID = TEXT comparison error
-- Note: ai_prompts.session_id is UUID, so we need to accept TEXT and cast it
...
COMMENT ON FUNCTION get_session_discovery_state IS 'Get discovery question state for current session (fixed UUID casting)';
